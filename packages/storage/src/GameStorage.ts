...
export interface Game {
  ...
  ownerId: string;
  stateVersion: number;
  gameState?: any; // JSON field for game-specific state
  createdAt: Date;
  updatedAt: Date;
  owner?: {
    id: string;
...
export interface GameMove {
  ...
  userId: string;
  sequence: number;
  moveData: any; // JSON field for game-specific move data
  timestamp: Date;
}
...
export abstract class GameStorage {
   ...
   */
  abstract getGame(gameId: string, options?: { includeOwner?: boolean; includePlayers?: boolean }): Promise<Game | null>;
   ...
   */
  abstract getGameMoves(gameId: string, options?: { limit?: number; offset?: number }): Promise<GameMove[]>;
   ...
   */
  abstract checkGameAccess(gameId: string, userId: string): Promise<{ canAccess: boolean; isOwner: boolean }>;
