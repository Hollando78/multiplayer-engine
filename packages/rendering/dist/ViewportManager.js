...
class ViewportManager {
    ...
    animationFrame = null;
    targetViewport = null;
    constructor(canvas, initialViewport, config = {}) {
        this.canvas = canvas;
    ...
    handleTouchEnd(e) {
        ...
        if (e.touches.length === 0) {
            ...
            if (this.isDragging && e.changedTouches.length === 1) {
                ...
                const touchData = this.touchData.get(touch.identifier);
                if (touchData) {
                    const distance = Math.sqrt(Math.pow(touch.clientX - rect.left - touchData.startX, 2) +
    ...
    startSmoothTransition() {
        if (!this.targetViewport || this.animationFrame)
            return;
        const animate = () => {
            if (!this.targetViewport)
                return;
