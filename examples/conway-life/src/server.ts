...
import { SocketManager, PubSubManager, ChunkUpdater, StateSync, defaultSocketConfig } from '@multiplayer-engine/realtime';
import { ConwayEngine } from './ConwayEngine';

// Game instances
const gameInstances: Map<string, ConwayEngine> = new Map();

// Real-time infrastructure
let socketManager: SocketManager;
let pubSubManager: PubSubManager;
let chunkUpdater: ChunkUpdater;
...
async function initializeServer(): Promise<void> {
  ...
  socketManager = new SocketManager(httpServer, socketConfig);

  // Initialize chunk-based updates system
  console.log('🔲 Setting up chunk update system...');
  chunkUpdater = new ChunkUpdater(socketManager, pubSubManager, 64);

  // Initialize state synchronization
  console.log('🔄 Setting up state synchronization...');
  stateSync = new StateSync(pubSubManager, socketManager, {
  ...
  socketManager.registerGameHandler('conway', {
    onPlayerJoined: async (socket: any, gameId: string) => {
    ...
    onCustomEvent: async (socket: any, event: string, data: any) => {
...
function setupRoutes(app: express.Application): void {
  ...
  app.post('/api/games', async (req: any, res: any) => {
    try {
      ...
      const game = new ConwayEngine(gameId);
  ...
  // Get game state
  app.get('/api/games/:gameId', async (req: any, res: any) => {
    try {
  ...
  // Place cell
  app.post('/api/games/:gameId/place', async (req: any, res: any) => {
    try {
      const { gameId } = req.params;
      ...
      if (result.success && result.changedCells) {
        ...
        await chunkUpdater.publishChunkUpdate(gameId, result.changedCells.map((cell: any) => ({
          x: cell.x,
          y: cell.y,
          oldValue: null,
          newValue: cell.owner,
...
function setupRealtimeHandlers(): void {
  ...
  socketManager.registerGameHandler('conway', {
    onCustomEvent: async (socket: any, event: string, data: any) => {
      if (event === 'viewport-changed') {
        ...
        try {
          const chunkIds = await chunkUpdater.subscribeToViewport(
            socket.id,
            gameId,
            viewport
          );
          ...
          socket.emit('error', { type: 'viewport-subscription-failed', error: (error as Error).message });
        }
      }
    }
  });
  ...
  pubSubManager.subscribeToAllGames(async (channel: any, message: any) => {
  ...
  socketManager.registerGameHandler('conway', {
    onPlayerLeft: async (socket: any, gameId: string) => {
 ...
 */
async function handlePlacePattern(socketId: string, data: any): Promise<void> {
  try {
 ...
 */
async function handleSetSpeed(socketId: string, data: any): Promise<void> {
  try {
